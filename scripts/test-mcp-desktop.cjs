...
(async () => {
  ...
  const unsavedMpf = await a.callTool({ name: 'vpx_mpf', arguments: { action: 'generate' } });
  assert(unsavedMpf.isError, 'MPF generate must require a saved table');
  const meshOut = await call(a, 'vpx_mesh', { action: 'export', partName: 'SmokeMesh' });
  assert(!meshOut.objPath.startsWith(root), `export landed in work dir: `);
  const tableOut = await call(a, 'vpx_geometry', { action: 'export_obj' });
  assert(!tableOut.objPath.startsWith(root), `export landed in work dir: `);
  assert(!(await fs.readdir(root)).includes('export'));
  console.log('SMOKE PASS exports stay out of the work dir');
  ...
  const mpf = await call(a, 'vpx_mpf', { action: 'generate' });
  assert.equal(mpf.configDir, path.join(temp, 'glf_mpf', 'config'));
  assert((await fs.readdir(mpf.configDir)).includes('switches.yaml'));
  assert(!(await fs.readdir(path.join(temp, 'Smoke_work'))).includes('glf_mpf'));
  console.log('SMOKE PASS MPF config written next to the saved table');
  const afterSave = await call(a, 'vpx_material', {
    action: 'add',
    material: { name: 'AfterSave' },
    confirm: true,
  });
  assert(afterSave.applied, JSON.stringify(afterSave));
  const otherAfterSave = await call(b, 'vpx_material', {
    action: 'add',
    material: { name: 'OtherAfterSave' },
    confirm: true,
  });
  assert(otherAfterSave.applied, JSON.stringify(otherAfterSave));
  const savedMaterials = await call(a, 'vpx_material', { action: 'list' });
  assert(savedMaterials.materials.some(m => m.name === 'AfterSave'));
  assert(savedMaterials.materials.some(m => m.name === 'OtherAfterSave'));
  console.log('SMOKE PASS continue editing after first save');
