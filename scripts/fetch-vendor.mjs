...
function listVbs(dir) {
  ...
  const walk = d => {
    for (const e of readdirSync(d)) {
      ...
      else if (p.endsWith('.vbs')) out.push(p);
    }
  };
  walk(dir);
...
function concatScript(root, spec) {
  const excluded = p => spec.exclude.some(x => p.replaceAll('\\', '/').includes(x));
  const chunks = [];
  for (const part of spec.parts) {
    const paths = part.dir ? listVbs(join(root, part.dir)) : [join(root, part.file)];
    for (const p of paths) {
      if (excluded(p) || !existsSync(p)) continue;
...
async function assembleVpxSource(source) {
  ...
  try {
    ...
    const members = [spec.subdir, ...(spec.inlineScript?.extract ?? [])].map(m => `${repoName}-${source.commit}/${m}`);
    execFileSync('tar', ['-xzf', 'repo.tgz', '--strip-components=1', ...members], { cwd: tmp });
    ...
    if (spec.inlineScript) {
      files['/vpx/script.vbs'] = Buffer.from(concatScript(tmp, spec.inlineScript), 'utf-8');
