...
type PositionsLike = ArrayLike<number>;

function boundingBox(positions: PositionsLike): BoundingBox {
  let minX = Infinity,
    minY = Infinity,
    minZ = Infinity;
  let maxX = -Infinity,
...
function centroid(positions: PositionsLike): Vec3 {
  let sx = 0,
    sy = 0,
    sz = 0;
...
function flatness(bbox: BoundingBox): number {
...
function xyCircularity(positions: PositionsLike, c: Vec3): number {
  const n = positions.length / 3;
  if (n < 8) return 0;
  let sum = 0;
  let sumSq = 0;
...
function cylindricality(positions: PositionsLike): number {
...
function discLikeness(positions: PositionsLike): number {
  const bbox = boundingBox(positions);
  const c = centroid(positions);
...
export function summarize(positions: PositionsLike): MeshSummary {
  const bbox = boundingBox(positions);
  const c = centroid(positions);
  return {
