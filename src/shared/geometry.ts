...
function translateDragPoints(points: DragPointLike[], dx: number, dy: number): DragPointLike[] {
  return points.map(p => ({ ...p, x: (p.x ?? 0) + dx, y: (p.y ?? 0) + dy }));
}

function scaleAround(points: DragPointLike[], center: Point, factor: number): DragPointLike[] {
  return points.map(p => ({
    ...p,
...
function rotateAround(points: DragPointLike[], center: Point, angleRad: number): DragPointLike[] {
