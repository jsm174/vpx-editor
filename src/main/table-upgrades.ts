...
type ConsoleOutputCallback = (type: string, text: string) => void;
...
export async function upgradeOldMaterialsFormat(dir: string): Promise<boolean> {
  ...
  if ((await fileExists(oldMatPath)) && !(await fileExists(newMatPath))) {
    const oldMaterials: OldMaterial[] = JSON.parse(await fs.promises.readFile(oldMatPath, 'utf-8'));
    const oldPhysics: OldPhysics[] = (await fileExists(oldPhysPath))
...
export async function upgradeLayersToPartGroups(
  ...
  const gameitems: GameItemInfo[] = JSON.parse(await fs.promises.readFile(gameitemsPath, 'utf-8'));
  ...
  for (const [initialGroupName, layerData] of layers) {
    ...
    const partGroupFileName = getFileNameFromItemName('PartGroup', groupName);
...
export async function cleanupCollectionItems(dir: string, sendConsoleOutput?: ConsoleOutputCallback): Promise<boolean> {
  ...
  const collections: Collection[] = JSON.parse(await fs.promises.readFile(collectionsPath, 'utf-8'));
  const gameitems: GameItemInfo[] = JSON.parse(await fs.promises.readFile(gameitemsPath, 'utf-8'));
