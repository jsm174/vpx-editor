...
function insertItem(itemType: string): void {
  const ctx = windowRegistry.getFocused();
  if (ctx) {
...
function createMenu() {
  ...
  const renameDialogOpen = !!windowStates?.renamePromptWindow;
  const dialogOpen =
    settingsOpen ||
    transformOpen ||
    aboutOpen ||
  ...
  const template = [
      ...
      submenu: [
        {
          ...
          click: (_menuItem: Electron.MenuItem, focusedWindow: BrowserWindow | undefined) => {
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          label: 'Script',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openScriptEditorWindow(),
        },
        {
          label: 'Backglass/POV',
          type: 'checkbox',
      ...
      submenu: [
        {
          ...
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Wall'),
        },
        {
          label: 'Gate',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Gate'),
        },
        {
          label: 'Ramp',
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Ramp'),
        },
        {
          label: 'Flasher',
          ...
          enabled: hasTable && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Flasher'),
        },
        {
          label: 'Flipper',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Flipper'),
        },
        {
          label: 'Plunger',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Plunger'),
        },
        {
          label: 'Bumper',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Bumper'),
        },
        {
          label: 'Spinner',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Spinner'),
        },
        {
          label: 'Timer',
          ...
          enabled: hasTable && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Timer'),
        },
        {
          label: 'Trigger',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Trigger'),
        },
        {
          label: 'Light',
          ...
          enabled: hasTable && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Light'),
        },
        {
          label: 'Kicker',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Kicker'),
        },
        {
          label: 'Target',
        ...
        },
        {
          ...
          enabled: hasTable && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Decal'),
        },
        {
          label: 'Textbox',
          ...
          enabled: hasTable && inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Textbox'),
        },
        {
          label: 'EM Reel',
          accelerator: 'CmdOrCtrl+Y',
          enabled: hasTable && inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Reel'),
        },
        {
          ...
          enabled: hasTable && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('LightSequencer'),
        },
        {
          label: 'Primitive',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Primitive'),
        },
        {
          label: 'Rubber',
          ...
          enabled: hasTable && !inBackglass && !isLocked && !dialogOpen,
          click: (_m: Electron.MenuItem, w: BrowserWindow | undefined) =>
            windowRegistry.getByWindow(w) && insertItem('Rubber'),
        },
      ],
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        {
          label: 'Table Info...',
          enabled: hasTable && !dialogOpen,
          click: () => windowFactory.openTableInfoWindow(),
        },
        {
          label: 'Sound Manager...',
          accelerator: 'F2',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openSoundManagerWindow(),
        },
        {
          label: 'Image Manager...',
          accelerator: 'F3',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openImageManagerWindow(),
        },
        {
          label: 'Material Manager...',
          accelerator: 'F4',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openMaterialManagerWindow(),
        },
        {
          label: 'Dimensions Manager...',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openDimensionsManagerWindow(),
        },
        {
          label: 'Collection Manager...',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openCollectionManagerWindow(),
        },
        {
          label: 'Render Probe Manager...',
          enabled: hasTable && !isLocked && !dialogOpen,
          click: () => windowFactory.openRenderProbeManagerWindow(),
        },
        { type: 'separator' },
        {
      ...
      submenu: [
        {
          ...
          enabled: !dialogOpen,
          click: () => windowFactory.openSettingsWindow(),
        },
      ],
    },
    {
    ...
    {
      label: 'Help',
      submenu: [{ label: 'About...', enabled: !dialogOpen, click: () => windowFactory.showAboutDialog() }],
    },
  ];
...
async function showBlueprintDialog(ctx: WindowContext, inBackglass: boolean) {
  nativeDialogOpen = true;
  windowRegistry.forEach(c => {
    c.window.webContents.send('set-input-disabled', true);
  });
  ...
  async (event, itemName: string, itemType: string, oldImage: string, newImage: string) => {
    ...
    try {
      ...
      const itemInfo = gameitems.find(
        (i: { file_name?: string }) => i.file_name && getItemNameFromFileName(i.file_name) === itemName
      );
...
ipcMain.on(
  'show-rename-dialog',
  async (event, data: { mode: string; currentName: string; existingNames?: string[] }) => {
    const ctx = windowRegistry.getContextFromEvent(event);
    if (!ctx) return;
    windowFactory.openRenamePromptWindow(ctx, data.mode, data.currentName, data.existingNames);
  }
);
...
ipcMain.on('rename-prompt-submit', async (_event, newName: string) => {
  ...
  const { ctx, mode, currentName } = renameData;
...
ipcMain.on('save-drawing-order', async (_event, data: { order: string[] }) => {
  ...
  try {
    ...
    const indicesToMove = data.order
      .map((name: string) => itemNameToIndex[name])
      .filter((i: number | undefined) => i !== undefined);
    if (indicesToMove.length >= 2) {
      const minIndex = Math.min(...indicesToMove);
      const movedItems = indicesToMove.map((i: number) => gameitems[i]);
      const newGameitems = gameitems.filter((_: unknown, i: number) => !indicesToMove.includes(i));
...
ipcMain.on('prompt-result', (_event, result: string | null) => {
  windowFactory.resolvePrompt(result);
});
...
ipcMain.handle('show-prompt', async (_event, config: { title?: string; message?: string; defaultValue?: string }) => {
  return await windowFactory.openPromptWindow(config);
});

ipcMain.on('info-result', () => {
  windowFactory.resolveInfo();
});

ipcMain.handle('show-info', async (_event, config: { title?: string; message?: string }) => {
  return await windowFactory.openInfoWindow(config);
});

ipcMain.on('confirm-result', (_event, result: { action: string }) => {
  windowFactory.resolveConfirm(result);
});

ipcMain.handle(
  'show-confirm',
  async (_event, config: { title?: string; message?: string; cancelAction?: string; options?: unknown[] }) => {
    return await windowFactory.openConfirmWindow(config);
  }
);

ipcMain.on('work-folder-result', (_event, result: string) => {
  windowFactory.resolveWorkFolder(result);
});
