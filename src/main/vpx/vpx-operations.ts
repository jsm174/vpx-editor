...
import { getLastFolder, setLastFolder, Settings } from '../settings/settings-manager.js';
...
function isUsingCustomVpxtool(settings: Settings): boolean {
  return !settings.useEmbeddedVpxtool && !!settings.vpxtoolPath;
}

function getVpxtoolPath(settings: Settings): string {
...
function spawnVpxtool(settings: Settings, args: string[], options: SpawnOptions): ChildProcess {
...
export async function extractVPX(vpxPath: string, options: ExtractOptions = {}, deps: ExtractDeps): Promise<void> {
  ...
  return new Promise<void>((resolve, reject) => {
    runVpxtoolExtract(
      ...
      async () => {
        const tempExtracted = path.join(tempDir, localCtx.tableName!);
        await fs.move(tempExtracted, workDir);
        await fs.promises.rm(tempDir, { recursive: true, force: true });

        sendConsoleOutput(localCtx, 'success', 'Extraction complete');
        ...
        await upgradeLayersToPartGroups(localCtx.extractedDir!, (type, text) =>
          sendConsoleOutput(localCtx, type, text)
        );
...
function runAssembleThenPlay(ctx: WindowContext, vpxPath: string, settings: Settings): void {
  playingContext = ctx;
  runVpxtoolAssemble(
    ctx,
    vpxPath,
