...
export const vpxEditorAPI = {
  ...
  onMcpReloadRequested: (cb: Function) => events.on('mcp-reload-requested', cb),
  onMcpRequest: (cb: Function) => events.on('mcp-request', cb),
  replyMcpRequest: (result: unknown) => events.emit('mcp-request-result', result),
  notifyConsoleReady: () => {},
  ...
  notifyScriptUndone: () => events.emit('script-undone'),
