...
export const position2 = z.object({ x: z.number(), y: z.number(), z: z.number().optional() });
const dragPoint = z
  .object({
    x: z.number(),
    y: z.number(),
...
type AddPartInput = z.infer<typeof addPartSchema>;
type ModifyPartInput = z.infer<typeof modifyPartSchema>;
...
export function extractModifyPartFields(input: ModifyPartInput): {
  ...
  const { type, partName, more, ...rest } = input as Record<string, unknown> & {
    type: string;
    partName: string;
    more?: Record<string, unknown>;
  };
