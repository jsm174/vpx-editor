...
import { runEdit, NO_ACTIVE_TABLE, isInsideWorkDir } from './edit-util.js';
...
const partInput = z.object({
  ...
  confirm: z
    ...
    .default(false)
    .optional()
    .describe('Required (true) for action="delete". Add and modify apply without it; use `preview` to dry-run those.'),
});
...
// The per-type schemas declare only a curated subset of fields; anything else the caller set at the
// top level would be silently stripped, so fold it back in and return the keys for verification.
...
const part: Tool<typeof partInput> = {
  ...
  async execute(input, ctx) {
    ...
    if (input.action === 'export') {
      ...
      if (handle && isInsideWorkDir(handle.workDir, input.outputPath))
        return errorResult('outputPath must be outside the table work folder');
      ...
      if (!parsed.success) {
        return errorResult(
          'action="add" needs `part.type` and `part.position`. ' +
            `Example: part: { type: "${input.part.type}", position: { x: 400, y: 1100, z: 0.5 } }. ` +
    ...
    if (input.action === 'modify') {
      ...
      if (!parsed.success) {
        return errorResult(
          'action="modify" needs `part.partName` (the name of the EXISTING part to change — not `name`) and `part.type`. ' +
            `Example: part: { type: "${input.part.type}", partName: "Disc1", position: { x: 326, y: 1100 } }. ` +
      ...
      return runEdit(
        ...
        {
          kind: 'modify-part',
          payload: { name: partName, patch: overrides },
