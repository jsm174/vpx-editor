...
import { grep, findSubs, findClasses, findConstants } from '../../../shared/vbs-analysis.js';
import { readScriptSelection } from './edit-util.js';
...
/** Top-level Class/Const names declared across the system scripts, for redefinition linting. */
...
export async function systemGet(
  ...
  const read = readScriptSelection(content, range ?? {}, {
    fullLimit: FULL_FILE_LIMIT,
    label: file,
    narrowHint:
  ...
  return 'error' in read ? errorResult(read.error) : jsonResult({ name: file, path: found, ...read.fields });
}
