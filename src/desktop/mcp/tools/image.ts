...
import { confirmable, runEdit, NO_ACTIVE_TABLE, filterByNameRegex } from './edit-util.js';
...
const image: Tool<typeof imageInput> = {
  ...
  async execute(input, ctx) {
    if (input.action === 'list') {
      ...
      const byName = filterByNameRegex(state.images, input.nameRegex);
      if ('error' in byName) return errorResult(byName.error);
      const images = byName.items;
