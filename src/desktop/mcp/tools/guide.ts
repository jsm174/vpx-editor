  ...
  async execute(_input, ctx) {
    ...
    return jsonResult({
      ...
      recommendedNext: hasTable
        ...
        : 'vpx_new(action:"create", start:"glf", name:"MyMachine") — creates a playable table to build on.',
      workflow: [
        {
          step: 1,
          do: 'Create a table (no table needs to be open).',
          command: 'vpx_new(action:"create", start:"glf", name:"MyMachine")',
          why: 'Gives you a minimal but already-playable GLF table: two flippers, a ball trough, and a plunger. Use start:"blank" for a bare table.',
        },
        {
          step: 2,
        ...
        },
        {
          ...
          command: 'vpx_mpf(action:"generate")',
          why: 'Writes an MPF config (switches/coils/lights/ball-devices) next to the table so the same design can drive a physical machine.',
        },
        {
          step: 6,
          do: 'Play it.',
