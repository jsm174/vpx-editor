...
const geometryInput = z.object({
  ...
  outputDir: z
    ...
    .optional()
    .describe(
      'For "export_obj": absolute directory for the files (default: an "export" folder in the table work dir).'
    ),
...
async function exportObj(ctx: ToolContext, outputDir: string | undefined, exchange: ObjExchangeOptions) {
  ...
  if (outputDir && !path.isAbsolute(outputDir)) return errorResult('outputDir must be absolute');
  ...
  const dir = outputDir ?? path.join(handle.workDir, 'export');
