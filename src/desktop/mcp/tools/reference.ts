...
const referenceInput = z.object({
  ...
  name: z
    ...
    .optional()
    .describe(
      `For action="patterns": specific pattern to fetch (omit to list; available: ${PATTERN_NAMES.join(', ')}). For action="glf_doc": the doc name, e.g. "drop-target".`
    ),
  file: z
    .string()
    .optional()
    .describe('For system_get (required, e.g. "core.vbs"); for system_search/system_summarize (optional filter).'),
  pattern: z.string().optional().describe('For action="system_search" or "glf_search": substring or regex to find.'),
  ...
  isRegex: z.boolean().default(false).optional().describe('For system_search/glf_search: treat pattern as regex.'),
  contextLines: z
    .number()
    .int()
    .min(0)
    ...
    .default(2)
    .optional()
    .describe('For system_search/glf_search: lines of context around each match.'),
  limit: z
    .number()
    .int()
    .positive()
    ...
    .default(80)
    .optional()
    .describe('For system_search/glf_search: max matches to return.'),
});
