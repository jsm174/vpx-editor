...
import { mpfConfigDir, writeMpfConfig, MPF_NEEDS_SAVED_TABLE } from '../library/mpf/write-config.js';
...
const mpf: Tool<typeof mpfInput> = {
  ...
  description:
    ...
    'next to the saved .vpx (darkchaos layout; "generate" and "get" require a saved table, "status" does not). ' +
  ...
  async execute(input, ctx) {
    ...
    if (input.action === 'get') {
      ...
      if (!dir) return errorResult(MPF_NEEDS_SAVED_TABLE);
    ...
    if (input.action === 'status') {
      return jsonResult({
        ...
        ...(mpfConfigDir(handle) ? {} : { note: MPF_NEEDS_SAVED_TABLE }),
