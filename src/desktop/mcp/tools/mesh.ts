...
const meshInput = z.object({
  ...
  outputDir: z
    .string()
    .optional()
    .describe('For "export": absolute directory for the files (default: an "export" folder in the table work dir).'),
  unit: objUnit
    .optional()
    .describe(
...
const mesh: Tool<typeof meshInput> = {
  ...
  async execute(input, ctx) {
    ...
    if (input.outputDir && !path.isAbsolute(input.outputDir)) return errorResult('outputDir must be absolute');
    const dir = input.outputDir ?? path.join(handle.workDir, 'export');
