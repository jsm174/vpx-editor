...
export type AuthFailureReason = 'missing' | 'mismatch';
...
export interface McpServerHandle {
  ...
  stop: () => Promise<void>;
  isRunning: () => boolean;
  lastAuthFailure: () => AuthFailure | null;
}
...
export async function startMcpServer(opts: StartOptions): Promise<McpServerHandle> {
  ...
  function authFailure(req: http.IncomingMessage): AuthFailureReason | null {
    const auth = req.headers['authorization'];
    const authStr = Array.isArray(auth) ? auth[0] : auth;
    if (!authStr) return 'missing';
    return authStr === `Bearer ${token}` ? null : 'mismatch';
  }
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (failure) {
      lastAuthFailure = { at: Date.now(), reason: failure };
      log(
        failure === 'missing'
          ? 'Rejected a connection with no access token. Open Tools → MCP Server → Settings… to set up the client.'
          : 'Rejected a connection with a wrong access token. The client may be using a token that was regenerated.'
      );
      ...
      res.end(
        JSON.stringify({
          ...
          hint:
            failure === 'missing'
              ? 'Send "Authorization: Bearer <token>". Copy the token from Tools → MCP Server → Settings… in VPX Editor.'
              : 'The access token does not match. Copy the current token from Tools → MCP Server → Settings… in VPX Editor.',
        })
      );
      return;
    }
