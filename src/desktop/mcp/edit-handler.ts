...
import { buildBundle, collectImageRefs, IMAGE_REF_KEYS, MATERIAL_REF_KEYS } from './library/bundle.js';
...
import { extractDonorAssets, cleanupDonor, readDonorImage, type DonorAssetSet } from './library/library-assets.js';
...
async function applyDeletePart(workDir: string, payload: Record<string, unknown>): Promise<EditResult> {
  ...
  await unlinkEditFile(path.join(workDir, 'gameitems', found.ref.file_name)).catch(() => undefined);
...
async function decodeImageSource(
  ...
  vpx: VpxReader | undefined
): Promise<DecodedBytes | { error: string }> {
  ...
  if (typeof source.base64 === 'string') {
    ...
    return { bytes, ext: extFromMime((source.mimeType as string) ?? 'image/png') };
  }
  if (source.libraryRef && typeof source.libraryRef === 'object') {
    const ref = source.libraryRef as { tablePath?: unknown; imageName?: unknown };
    if (typeof ref.tablePath !== 'string' || typeof ref.imageName !== 'string') {
      return { error: 'libraryRef needs {tablePath, imageName}.' };
    }
    if (!vpx) return { error: 'libraryRef image source needs the vpx reader, which is unavailable.' };
    const donor = await readDonorImage(ref.tablePath, vpx, ref.imageName);
    if ('error' in donor) return donor;
    return { bytes: donor.bytes, ext: extFromMime(donor.mimeType) };
  }
...
async function applyAddImage(
  ...
  const decoded = await decodeImageSource(source, vpx);
...
async function applyModifyImage(
  workDir: string,
  payload: Record<string, unknown>,
  vpx: VpxReader | undefined
): Promise<EditResult> {
...
async function decodeSoundSource(source: Record<string, unknown>): Promise<DecodedBytes | { error: string }> {
  if (typeof source.path === 'string') {
...
async function applyCloneBundle(deps: DirectEditDeps, payload: Record<string, unknown>): Promise<EditResult> {
  ...
  // donorState comes from the caller (ctx.loadTable); donorTablePath re-extracts binary assets below.
  ...
  try {
    ...
    const partImageRefs = collectImageRefs(bundle.partData);
    ...
    if (donorAssets) {
      for (const refName of neededImages) {
        ...
        const result = await applyAddImage(
          deps.workDir,
          { name: newName, source: { base64: asset.bytes.toString('base64'), mimeType: asset.mimeType } },
          deps.vpx
        );
    ...
    const rewritten = rewriteRefs(retargeted, materialNameMap, imageNameMap);
...
export async function applyEditDirect(deps: DirectEditDeps, op: EditOperation): Promise<EditResult> {
  try {
    switch (op.kind) {
      ...
      case 'add-image':
        return applyAddImage(deps.workDir, op.payload, deps.vpx);
      case 'modify-image':
        return applyModifyImage(deps.workDir, op.payload, deps.vpx);
      case 'delete-image':
