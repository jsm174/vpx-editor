...
import { findItem, loadFromWorkFolder, type TableState } from '../../shared/table-state.js';
...
export interface ContextDeps {
  ...
  /** Save the table in the given window. With a path, saves there directly; otherwise uses the editor's normal save flow (may open a native Save As dialog). */
  saveTable: (windowId: string, targetPath?: string) => Promise<{ saved: boolean; error?: string }>;
...
async function applyEditAndNotifyRenderer(
  ...
  try {
    return await directTransaction(
      ctx,
      deps,
      op.description,
...
async function findPrimitive(
  ...
  const state = await loadFromWorkFolder(workDir);
...
export function resolveSaveTarget(target: string, tableName: string | null): { path: string } | { error: string } {
  const trimmed = target.trim();
  if (!path.isAbsolute(trimmed)) {
    return {
      error: `"${target}" is not an absolute path. Pass the full path of the folder you are working in (or a full .vpx path).`,
    };
  }
  if (/\.vpx$/i.test(trimmed)) return { path: path.normalize(trimmed) };
  const name = (tableName ?? '').trim() || 'untitled';
  return { path: path.join(trimmed, `${name}.vpx`) };
}
...
export function createToolContext(deps: ContextDeps): ToolContext {
  ...
  function resolve(): { ctx: WindowContext | null; error?: string } {
    if (boundWindowId) {
      ...
      if (ctx && ctx.window && !ctx.window.isDestroyed() && ctx.hasTable() && ctx.tableGeneration === boundGeneration)
        return { ctx };
      return {
        ctx: null,
        error:
    ...
    if (ctx) {
      ...
      boundGeneration = ctx.tableGeneration;
      return { ctx };
    }
    return { ctx: null };
  }
  ...
  async function withTable<T>(fn: () => Promise<T>): Promise<T> {
    ...
    const generation = ctx.tableGeneration;
    return serializeTable(ctx.extractedDir, async () => {
      if (resolve().ctx !== ctx || ctx.tableGeneration !== generation)
  ...
  const context: ToolContext = {
    ...
    async saveTable(target?: string): Promise<{ saved: boolean; path: string | null; error?: string }> {
      const { ctx, error } = resolve();
      if (!ctx || !ctx.extractedDir) {
      ...
      if (target) {
        const resolved = resolveSaveTarget(target, ctx.tableName);
        if ('error' in resolved) return { saved: false, path: ctx.currentTablePath, error: resolved.error };
        targetPath = resolved.path;
      }
      const hadPath = !!ctx.currentTablePath || !!targetPath;
      try {
        const result = await deps.saveTable(ctx.id, targetPath);
        if (!result.saved) {
          return {
            saved: false,
    ...
    async playTest(opts: { seconds: number }): Promise<PlayTestResult> {
      const { ctx, error } = resolve();
      if (!ctx || !ctx.extractedDir) return failPlayTest(error ?? 'No active table');
      const vpinballPath = deps.getVpinballPath();
      if (!vpinballPath) {
        return failPlayTest('VPinballX path is not configured. Ask the user to set it in Preferences > Paths.');
      }
      try {
        const vpin = await deps.initVpinModule();
        ...
        return failPlayTest(err instanceof Error ? err.message : String(err));
      }
    },
    vpx,
