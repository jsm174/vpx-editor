...
export interface ToolContext {
  ...
  /** Load a donor .vpx from an absolute path on demand. */
  ...
  /** Save the active table. `target` is an absolute folder or .vpx path to save to directly; without it the editor's normal save flow runs (may open a native Save As dialog). */
  saveTable(target?: string): Promise<{ saved: boolean; path: string | null; error?: string }>;
