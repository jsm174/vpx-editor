...
async function resolveVpinballExecutable(vpinballPath: string): Promise<string | null> {
...
export function failPlayTest(error: string): PlayTestResult {
  return {
    ok: false,
    ranSeconds: 0,
    exitCode: null,
...
export async function runPlayTest(opts: PlayTestOptions): Promise<PlayTestResult> {
  ...
  if (launchError) return failPlayTest(`Could not launch VPinballX (${vpinballPath}): ${launchError}`);
