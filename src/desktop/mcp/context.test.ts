...
import { createToolContext, type ContextDeps } from './context.js';
...
it('rolls back writes and releases the editor after renderer failure', async () => {
  ...
  await expect(createToolContext(deps).applyEdit(material('failed'))).rejects.toThrow('renderer failed');
...
it('does not overwrite files when pending manual script changes cannot be saved', async () => {
  ...
  await expect(createToolContext(deps).applyEdit(material('wrong'))).rejects.toThrow('manual save failed');
  expect(await fs.readdir(root)).toEqual([]);
  expect(win.mcpEditBusy).toBe(false);
  expect(win.mcpEditWriting).toBe(false);
});
