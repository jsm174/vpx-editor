...
import { applyEditDirect, rewriteRefs } from './edit-handler.js';
import { collectImageRefs } from './library/bundle.js';
...
describe('applyAddImage vpin round-trip', () => {
  ...
  it('copies an image out of a donor table via libraryRef', async () => {
    const donorPath = path.join(workDir, 'donor.vpx');
    await fs.promises.writeFile(donorPath, 'not a real vpx');
    const png = Buffer.from(PNG_BASE64, 'base64');
    const vpx = {
      extract: async () => ({
        '/vpx/images.json': new TextEncoder().encode(JSON.stringify([{ name: 'Logo', path: 'images/Logo.png' }])),
        '/vpx/images/Logo.png': new Uint8Array(png),
      }),
    };
    const result = await applyEditDirect(
      { workDir, vpx },
      {
        kind: 'add-image',
        payload: { name: 'Borrowed', source: { libraryRef: { tablePath: donorPath, imageName: 'Logo' } } },
        description: 'add',
      }
    );
    expect(result.success).toBe(true);
    expect(await fs.promises.readFile(path.join(workDir, 'images', 'Borrowed.png'))).toEqual(png);

    const missing = await applyEditDirect(
      { workDir },
      {
        kind: 'add-image',
        payload: { name: 'Nope', source: { libraryRef: { tablePath: donorPath, imageName: 'Logo' } } },
        description: 'add',
      }
    );
    expect(missing.success).toBe(false);
    expect(missing.error).toMatch(/vpx reader/);
  });
...
describe('image ref keys', () => {
  it('does not treat image_alignment as an image reference', () => {
    ...
    expect(collectImageRefs(part).sort()).toEqual(['flash', 'wrap']);

    const imageMap = new Map([['wrap', 'Cloned_wrap']]);
    const rewritten = rewriteRefs(part, new Map(), imageMap);
